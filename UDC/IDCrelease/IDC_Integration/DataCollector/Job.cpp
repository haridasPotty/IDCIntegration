...
namespace DataPointConfiguration
{
    ...
    bool DataPointConfigurationHandler::getTriggerAnnotationIDs( \
                            std::vector<uint32_t>&  annotationIDHolder){
                            annotationIDHolder = this->triggerAnnotationIDs;
        return true;
    }
    bool DataPointConfigurationHandler::getDataPointConfiguration( \ 
                                        CollectionConfiguration configType,\
    ...
    bool DataPointConfigurationHandler::CheckForPeriodicity(){
        if (this->triggerAnnotationIDs.empty() ==true &&  \ 
                         this->event_trigger.empty() == true ){ 
            return true;
        }
        return false;
    }
...
bool DataPoints::updateDatapoint(std::string sourceName){
    dataPoints.push_back(sourceName);
    return true;
    
}
bool DataPoints::getDataPoint(std::string& datasource){
    if (isDataPointEmpty() != true){
        datasource = dataPoints.back();
        dataPoints.pop_back();
        return true;
    }
    return false;
...
bool DataPoints::isDataPointEmpty(){
    if (dataPoints.empty() ){
        return true;
    }
    return false;
